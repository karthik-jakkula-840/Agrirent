...
import { Users, FileText, ExternalLink, ShieldCheck } from 'lucide-react'
...
export default async function OwnerRequestsPage() {
  ...
  const isValidDoc = (url?: string | null) => {
    return url && url !== '#' && url !== 'null' && (url.startsWith('http') || url.startsWith('/'))
  }
      ...
      <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500 mt-1 text-sm font-medium">Review and approve applications and Aadhaar cards for equipment owners.</p>
          </div>
        </div>
      </div>
          ...
          <>
            ...
            <div className="divide-y divide-gray-100 md:hidden">
              {requests.map((req: any) => {
                const hasAadhaar = isValidDoc(req.identity_document_url)
                ...
                return (
                  <div key={req.id} className="p-4 space-y-3">
                    ...
                    <div className="text-xs space-y-1 text-gray-600 bg-gray-50 p-2.5 rounded-xl">
                      ...
                      <div className="truncate"><span className="text-gray-400">Details:</span> {req.business_address}</div>
                    </div>
                      ...
                      <div>
                        {hasAadhaar ? (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200 rounded-xl text-xs font-bold transition-colors"
                          >
                            <ShieldCheck className="h-3.5 w-3.5 text-emerald-600" />
                            <span>View Aadhaar ↗</span>
                          </a>
                        ) : (
                          <span className="text-xs text-gray-400 italic">No Aadhaar uploaded</span>
                        )}
                      </div>
              ...
              <table className="w-full text-sm text-left">
                <thead className="bg-gray-50/50 text-gray-500 font-medium border-b border-gray-100">
                  <tr>
                    ...
                    <th className="px-6 py-4">Business / Notes</th>
                    <th className="px-6 py-4">Aadhaar / ID Proof</th>
                    ...
                    return (
                      <tr key={req.id} className="hover:bg-gray-50/50 transition-colors">
                        <td className="px-6 py-4">
                          ...
                          <div className="text-xs text-gray-500 font-mono">{req.user?.phone || 'N/A'}</div>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          {hasAadhaar ? (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200 rounded-xl text-xs font-bold transition-all shadow-xs"
                            >
                              <ShieldCheck className="h-3.5 w-3.5 text-emerald-600" />
                        ...
                        <td className="px-6 py-4">
                          <Badge variant="outline" className={`capitalize font-medium text-xs px-2.5 py-0.5 rounded-full ${
                            req.status === 'approved' ? 'bg-green-50 text-green-700 border-green-200' :
                            req.status === 'rejected' ? 'bg-red-50 text-red-700 border-red-200' :
                            'bg-amber-50 text-amber-700 border-amber-200'
                          }`}>
